...
(function(_, url_map, undefined) {
    ...
    function require(arr, onready) {
        ...
        var left = arr.length;
        if (!left && onready) {
            return onready.apply(_, findAll(arr));
        }

        function check() { 
            if (!--left && onready) {
                onready.apply(_, findAll(arr));
            }
        }

        return each(arr, function(req) {
            return request(req, check);
        });
    ...
    require.setObjUrl = setObjUrl;
    require.addObjMap = addObjMap;
